...
namespace ParallelLUDecomposition
{
   public partial class Form1 : Form
   {
      ...
      private void voBtnSequential_Click( object aoSender, EventArgs aoArgs )
      {
         ...
         koSW.Start( );
         koM.MLUDecompose( kdL, kdU );
         koSW.Stop( );

         /// -# Print the error
         MessageBox.Show( "Error = " + kdErr.ToString( ) + Environment.NewLine +
                          "Milliseconds = " + koSW.ElapsedMilliseconds.ToString( ) );
      ...
      private void voBtnParallel_Click( object sender, EventArgs e )
      {
         int         kiSize = 8;
         double[ , ] kdL  = new double[ kiSize, kiSize ];
         double[ , ] kdU  = new double[ kiSize, kiSize ];
