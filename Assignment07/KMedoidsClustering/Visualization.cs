...
namespace KMedoidsClustering
{
   class Visualization
   {
      public static bool MDrawClusters( PictureBox aoPB, List< Point > aoPList, List< int > aoClusterInfo, int aiClusterCount, List< int > aiMedoids, double adScale = 1 )
      {
         ...
         koG.DrawLine( new Pen( koCColors[ aoClusterInfo[ 0 ] ], 1 ), koP1a, koP2a );     
         koG.DrawLine( new Pen( koCColors[ aoClusterInfo[ 0 ] ] ), koP3a, koP4a );        

         koPStart = new Point( koP1.X, koP1.Y );       
         for ( int kiI = 1; kiI < aoPList.Count; kiI++ )           
         {                
            if( aiMedoids.Contains( kiI ) )
            {
               koP2 = aoPList[ kiI ];
               koG.DrawEllipse( new Pen( koCColors[ aoClusterInfo[ kiI ] ], 1 ), ( int )( koP2.X * adScale ) - 2, ( int )( koP2.Y * adScale ) - 2, 4, 4 );
            }
            else
            {
               koP2 = aoPList[ kiI ];     
               ...
               koG.DrawLine( new Pen( koCColors[ aoClusterInfo[ kiI ] ], 1 ), koP1a, koP2a );                     
               koG.DrawLine( new Pen( koCColors[ aoClusterInfo[ kiI ] ] ), koP3a, koP4a );
            }
         }             
      ...
      public static bool MDrawX( Graphics aoG, Point aoPt1 )
      {
         ...
         aoG.DrawLine( new Pen( Color.Red, 3 ), koP1, koP2 );
         aoG.DrawLine( new Pen( Color.Red, 3 ), koP3, koP4 );
